...
class BillOutboxEventProcessor {

    private final BillWebsocketSessionRegistry websocketSessionRegistry;
    private final ObjectMapper om;

    @Transactional(Transactional.TxType.REQUIRES_NEW)
    void processEvent(BillOutboxEvent event) {
        try {
            var deserialize = om.readTree(event.getPayload());
            var msgObj = new BillPayloads.BillWebsocketMessage(
                    event.getType().name(),
                    deserialize
            );
